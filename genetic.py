...
initial_populations = [[
        Population(
            population_size,
            [Individual(slots_cnt, indiv.alloc) for indiv in population.curr_population]
...
for config_i in range(len(settings)):
    ...
    for test_i in range(TESTS):
        ...
        for i in range(GENERATIONS):
            ...
            RWS_weights = [working_population[allocIndex].fitness+1 for allocIndex in range(population_size)]
            if not (sum(RWS_weights) > 0):
                RWS_weights = [1 for _ in range(population_size)] # give every alloc an equal weight if every alloc is over budget
