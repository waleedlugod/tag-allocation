...
influences = [float(_.replace("\x00", "")) for _ in influences]
plt.figure()
plt.hist(influences)
...
raw_influences = [float(_.replace("\x00", "")) for _ in raw_influences]
plt.figure()
...
# note: graph is right skewed as not all users contribute to the final influence value (i.e. 0 value)
influences = open("influences_all.txt", "r").read().splitlines()
influences = [float(_.replace("\x00", "")) for _ in influences]
plt.figure()
plt.hist(influences)
plt.xlabel("Influence")
...
raw_influences = open("raw_influences_all.txt", "r").read().splitlines()
raw_influences = [float(_.replace("\x00", "")) for _ in raw_influences]
plt.figure()
plt.hist(raw_influences)
plt.xlabel("Influence")
